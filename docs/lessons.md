...
* **Stutter with a Logitech receiver keyboard: the phantom battery.** Gaming Mode stuttered and the
  system reported a battery at "<1 %" on a box that has none. The kernel's
  ...
  manager takes it for the system's, polls it every two seconds (each poll a stall on a device that
  does not answer, which was the stutter) and shows the low-battery state that comes with it. The driver
